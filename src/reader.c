...
int main()
{
   ...
   shared_mem_id = shmget(shared_mem_key, BUFFER*sizeof(int), IPC_CREAT | 0666);
   if (shared_mem_id == -1) {
   ...
   fprintf(stderr, "$ Reader %d starts printing\n", getpid());
   read_num_from_file(FILENAME);
   fprintf(stderr, "$ Reader %d finished printing\n", getpid());
